...
| Human labeling and judge alignment | `ACTIVE` | All 50 edge-v2 observations are structurally valid and oracle-bearing. evaluatorq replayed all 50 with no target inference through pinned `analytics-answer-correctness@1.0.0` and uploaded one Experiment. Human labels and stability outputs do not yet exist | Materialize development-only alignment inputs, estimate repeated-call cost, and obtain approval before stability runs |
...
The most recent fresh offline evidence is `83 passed, 1 deselected` plus Ruff from the reproducible corpus-generator change; final integrated verification is still required after the current adapter changes. Resource loading found one agent, two tools, two deterministic evaluators, and four human-label-gated LLM evaluators. The baseline 50-case input corpus reproduced exactly from DuckDB oracles with a 30-dev/20-test split. A project-scoped run key exists only in the primary ignored `.env`, and its exact run-key preflight confirmed the hosted agent is live. The one-time edge-v2 run created 50 distinct observations: eight one-turn, 22 two-turn, and 20 three-turn outputs; 40 achieved their goal. All ten behavioral failures are retained, while five missing expected-save calls are QC warnings rather than structural rejection reasons.
...
- [x] WS5: The focused runner uses evaluatorq `inference=False`, no jobs, one pinned answer-correctness scorer, and uploaded all 50 baseline Experiment rows without locally printing results.
...
No workstream is currently confirmed blocked. The two conditions recorded as blockers earlier on 2026-09-05 were both resolved in the same task: `analytics-answer-correctness` was applied to `pydata2026` in `shadow` status at version `1.0.0`, and the `.env` project key was confirmed working through the SDK sync path. WS5 still depends on human labels and cost approval, which are gates rather than blockers.
