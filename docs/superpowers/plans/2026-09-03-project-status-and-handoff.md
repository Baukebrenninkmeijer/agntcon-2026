...
| Human labeling and judge alignment | `ACTIVE` | All 50 edge-v2 observations are structurally valid and oracle-bearing. evaluatorq replayed all 50 with no target inference through pinned `analytics-answer-correctness@1.0.0`. Upload routing is blocked by production bug BOPS-1180: `path="pydata2026"` is accepted but the Experiment is still assigned to Default | Wait for BOPS-1180 to leave Testing before another placement retry; meanwhile materialize development-only alignment inputs and estimate repeated-call cost |
...
- [x] WS5: The focused runner uses evaluatorq `inference=False`, no jobs, one pinned answer-correctness scorer, forwards an explicit Experiment path, and uploaded every 50-row baseline execution without locally printing results.
...
Because the Experiment is absent from the `pydata2026` sidebar, use the direct link to the latest uniquely named 50-row run: [pydata2026 answer-correctness baseline 1.0.0](https://my.orq.ai/<workspace>/experiments/01M1RGTTZV0XPJ1DWHE0EASXYZ?runId=01M1RGTTZTBA7FSQV07K85HGG8).
...
|---|---|---|---|
| 2026-09-05 (9, latest) | WS5 | Added the direct Orq URL for the latest uniquely named 50-row correctness baseline to the README and living handoff because BOPS-1180 prevents discovery under the intended project | Use the direct link until production project-path routing is fixed |
...
1. WS5 owner: materialize the 30 development rows into the evaluator-alignment run-directory contract without exposing test rows to prompt selection.
2. WS5 owner: fetch the pinned baseline evaluator, estimate the repeated stability job, and obtain explicit approval before paid repetitions.
3. WS5/user: review the unstable groups and stable spot-checks, provide human rules/labels, and approve any proposed prompt rewrite before a remote evaluator update.
4. WS5 owner: after the updated version exists, run both explicit pinned versions together on identical rows so evaluatorq uploads side-by-side score columns; never resolve `latest`.
5. Coordinator: run final offline verification, review for secrets/opaque IDs, commit, and publish the coherent change set under the user's standing push instruction.
