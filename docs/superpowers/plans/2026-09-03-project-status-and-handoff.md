...
| Human labeling and judge alignment | `ACTIVE` | All 50 edge-v2 observations are structurally valid and oracle-bearing. A corrected 50-row evaluatorq replay through pinned `answer_correctness@1.0.0` returned and exported 50/50 populated scores: 47 pass and three fail. The validated local JSONL preserves all results for alignment. The earlier SDK-backed 0/50 run remains invalid evidence | Materialize the 30-row development alignment input and begin human-label comparison; placement still remains blocked by BOPS-1180 |
...
1. WS5 owner: materialize the 30 development rows from the validated local baseline into the evaluator-alignment run-directory contract without exposing test rows to prompt selection.
