...
- [ ] WS5: Run the approved stability job (`stability.py`, 50 × 8 at temperature 1, ~400 judge calls) and report per-row instability plus the oracle-graded correctness block.
...
1. WS5/user: the alignment run directory currently holds all 50 rows (dev and test). Decide whether to cut it to the 30 dev rows before any grey-zone labelling so test rows stay unseen by prompt selection; stability measurement alone does not expose them.
2. WS5 owner: the pinned evaluator (`1.0.7`) is fetched and the stability job is estimated (50 × 8, ~400 calls); obtain explicit approval before paid repetitions. Drive the repetitions from evaluatorq (N invokes of `analytics-answer-correctness@<version>` per row, each verdict persisted); leave the hosted `repetitions: 1`, which returns only a majority verdict.
