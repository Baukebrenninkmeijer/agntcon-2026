...
| Hosted resources and simulation | `VERIFIED` | The hosted agent is live in `<workspace>/pydata2026`. The baseline and harder edge-v2 runs each produced 50 unique raw outputs. Edge-v2 reached two or more turns in 42/50 rows and retained ten behavioral failures | Preserve the frozen raw artifacts; do not rerun |
...
| Human labeling and judge alignment | `ACTIVE` | All 50 edge-v2 observations are structurally valid and oracle-bearing. The latest 50-row evaluatorq replay is invalid: its export has 0/50 populated `answer_correctness@1.0.0` scores because orq-ai-sdk 4.14.7 discarded valid top-level v3 evaluator responses. The scorer now uses a shared async HTTP client; a three-row evaluatorq smoke run returned and exported 3/3 populated pinned scores with no errors | Run one fresh uniquely named 50-row replay, then require 50/50 non-empty exported scores before alignment; placement still remains blocked by BOPS-1180 |
...
The latest integrated-tree verification is `108 passed, 1 deselected`, Ruff clean, `git diff --check` clean, and successful sdist/wheel build on 2026-09-05. Resource loading found one agent, two tools, two deterministic evaluators, and four LLM evaluator definitions; only `analytics-answer-correctness` is applied remotely. The ignored `.env` key has all-project scope and the Orq CLI session selects Default; do not describe it as project-scoped. The one-time edge-v2 run created 50 observations: eight one-turn, 22 two-turn, and 20 three-turn outputs; 40 achieved their simulation goal. All ten behavioral failures and five missing-save warnings remain replayable. Three pinned `1.0.0` correctness executions uploaded 50 rows each across two Experiment objects, but production BOPS-1180 assigned both objects to Default even when the latest unique-name run explicitly sent `path="pydata2026"`. The latest run's exported evaluator column is empty in all 50 rows and must not be used as baseline evidence.
...
3. [Edge-case simulation v2 design](../specs/2026-09-05-edge-case-simulation-v2-design.md) — frozen hard-corpus provenance, one-run policy, observed evidence, and bounded QC-filtering contract.
4. [Analytics chatbot design](../specs/2026-09-02-analytics-chatbot-design.md) — local execution, safety, dataset, trace attribution, and public API boundaries, subject to its hosted-agent/evaluation revision note.
5. [Analytics chatbot implementation plan](2026-09-02-analytics-chatbot.md) — completed core implementation history.
6. [Post-hoc trace evaluation design](../specs/2026-09-02-posthoc-trace-evaluation-design.md) — `SUPERSEDED` for evaluator execution, service, CLI loop, and evaluation ledger. Consult only for still-relevant read-only mapping and provenance concerns.
7. [Original post-hoc implementation plan](2026-09-02-posthoc-trace-evaluation.md) — `SUPERSEDED`; do not execute it as written.
...
| WS3b | `row-aware-simulation-corpus` | `ACTIVE` | Local-tool bridge, baseline definitions/observations, and a separate 50-row harder edge-v2 observation set | WS3a hosted/local protocol; WS0 data/oracles | WS2 after row contract agreement |
...
- [ ] WS5: Run a fresh uniquely named 50-row replay and verify its exported correctness column is populated in all 50 rows before treating it as a baseline.
...
The earlier evaluator-resource blocker remains resolved: `analytics-answer-correctness` is applied to `pydata2026` in `shadow` status at version `1.0.0`, and the `.env` key works through the SDK sync path. WS5 still depends on human labels and cost approval, which are gates rather than blockers.
...
| 2026-09-05 | Exclude `multi_turn_consistency` from alignment and gating | Accepted; only eight of 50 observations are multi-turn, so its locked thresholds are unreachable. Code, routing, and YAML are retained unchanged as `NOT ALIGNABLE` |
...
1. WS5 owner: run one fresh uniquely named 50-row replay through the pinned evaluator using the async HTTP scorer, export the Experiment, and require 50/50 populated correctness scores.
2. WS5 owner: materialize the 30 development rows into the evaluator-alignment run-directory contract without exposing test rows to prompt selection.
3. WS5 owner: fetch the pinned baseline evaluator, estimate the repeated stability job, and obtain explicit approval before paid repetitions.
4. WS5/user: review the unstable groups and stable spot-checks, provide human rules/labels, and approve any proposed prompt rewrite before a remote evaluator update.
5. WS5 owner: after the updated version exists, run both explicit pinned versions together on identical rows so evaluatorq uploads side-by-side score columns; never resolve `latest`.
6. Coordinator: run final offline verification, review for secrets/opaque IDs, commit, and publish the coherent change set under the user's standing push instruction.

Do not begin the 50 x 3 stability baseline until the trace adapter, hosted/local bridge, frozen corpus, human labels, and offline validation gates are all accepted.
