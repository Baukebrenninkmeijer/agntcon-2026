...
| Hosted resources and simulation | `VERIFIED` | The hosted agent is live in `<workspace>/pydata2026`. The baseline, harder edge-v2, and single-persona v3 runs each produced 50 unique raw outputs. v3 (one `business-analyst` persona, 50 distinct situations) was run twice: the first run had gateway PII masking enabled and is archived as `*.pii-masked.*`; the accepted rerun reached two or more turns in 28/50 rows, achieved 49 goals, and retained one behavioral failure. Those observations are retained historical evidence; v4 currently has definitions only | Preserve the frozen edge-v2/v3 artifacts and do not rerun them. Generate v4 observations only after separate authorization |
