...
| Decision-support evaluation implementation | `ACTIVE` | Tasks 1-4 are implemented in the isolated `feature/decision-support-evaluation` worktree. The tracked `simulation-v4` review pool contains 50 context-enriched Sphere situations with the preserved v3 30/20 split. The replay row preserves case-authored decision context, and the local and repository-resource contracts now expose one reference-free `decision_support_quality` jury with the approved three models, all-model assignment, majority aggregation, minimum two successful judges, and three repetitions. The local and hosted agent prompts use the same rudimentary Sphere.com baseline without decision coaching. Task 4 passed 23 focused tests and the full non-live suite (120 passed, one deselected); Ruff and offline resource compilation are clean. No hosted apply, evaluator invocation, dependency change, or historical corpus/run mutation occurred | Review and integrate Tasks 1-4 on local `main`, rerun the offline gate, then stop at the Task 5 evaluatorq dependency gate |
...
| Hosted resources | `ACTIVE` | Task 4's isolated branch replaces the four old subjective YAML definitions with one pending-human-labels `analytics-decision-support-quality` jury, retains the two historical Python evaluator files unchanged, and reduces both agent prompts to the rudimentary Sphere.com baseline. Offline compilation found one agent, two tools, two Python evaluators, and one subjective evaluator. The historical hosted agent/tool path and applied `analytics-answer-correctness@1.0.8` remain remote history; no Task 4 apply is authorized or occurred | Review and integrate the local resource change, then obtain separate authorization before any authenticated semantic plan or hosted apply |
...
| Human labeling and judge alignment | `ACTIVE` | Edge-v2/v3 `answer_correctness` replays, stability results, and alignment artifacts are retained historical evidence of the superseded correctness-first increment; they do not authorize more correctness work. The accepted current increment is v4 `decision_support_quality`: 50 definitions with a fixed 30-dev/20-test split exist, but no v4 observations or human labels exist yet. The planned three-judge, three-repetition jury will use disagreement and within-model wobble only to prioritize human annotation | First obtain accepted v4 observations. Jury calls then require both an evaluatorq release that returns the detailed `assignment="all"` payload and explicit call approval; preserve the frozen test split and human-label authority |
| Decision-support jury run and post-hoc operations | `NOT STARTED` | No v4 jury call has run. The approved full shape is 50 rows × 3 judges × 3 repetitions = 450 calls, preceded by a two-row smoke; there is no separate 50×3 stability baseline | Requires accepted v4 observations, a released evaluatorq detailed-jury payload, successful smoke evidence, and explicit paid-call approval |
...
| WS6 | `jury-execution-and-operations` | `NOT STARTED` | Two-row DSQ jury smoke, then the separately approved 50-row × 3-judge × 3-repetition run (450 calls), protected canary, post-hoc sampling, and reviewed prompt loop | Accepted v4 observations; released detailed-jury evaluatorq package; explicit call approval; later WS5 qualification for post-hoc promotion | Offline maintenance |
...
- [ ] Decision-support Task 4 is implemented in the same isolated branch: local and hosted prompts now share the rudimentary Sphere.com baseline; one pending `analytics-decision-support-quality` YAML replaces the four old LLM definitions; subjective guards require full-conversation/final-response evidence and reject reference-family inputs or claims. TDD RED failed 9 focused tests, GREEN passed 23 focused tests and 120 non-live tests with one deselection, and offline compilation loaded the expected 1/2/2/1 agent/tool/Python/LLM inventory. The authenticated dry-run command has no compile-only mode and its remote snapshot preflight was blocked by DNS, so it produced no remote semantic plan and no apply occurred. Integration plus fresh offline verification are still required.
...
No current workstream is classified `BLOCKED`; the v4 observation, released detailed-jury package, and explicit call-approval requirements remain dependency gates. Historical Experiment placement under `pydata2026` failed because of [BOPS-1180](https://linear.app/orqai/issue/BOPS-1180/evaluatorq-experiments-ignore-path-always-land-in-the-default-project): on 2026-09-05, same-name and unique-name correctness retries both sent `path="pydata2026"`, but direct API reads showed Default's project ID and no experiment under `pydata2026`. Before any future DSQ Experiment, verify the backend fix with the two-row v4 smoke and its returned `project_id`; do not make that smoke call until all current gates are satisfied.
...
The earlier evaluator-resource blocker remains resolved as historical state: `analytics-answer-correctness` is applied to `pydata2026` in `shadow` status at version `1.0.8` (reference-free single judge), and the `.env` key works through the SDK sync path. This does not make correctness the current evaluator. The v4 `decision_support_quality` increment still has no hosted definition, observations, jury result, or human labels; its evaluatorq detailed-payload dependency and explicit call approval are gates rather than blockers.
...
- [ ] After the released detailed-payload gate and a successful two-row smoke, obtain explicit approval for the 50-row × 3-judge × 3-repetition `decision_support_quality` jury (450 calls) and preserve its complete vote record.
...
- This is isolated-branch evidence, not integrated proof. M1 remains `ACTIVE` until the reviewed Task 3 commit is integrated into local `main` and the focused plus full non-live test/lint gates pass again on the integrated tree.
...
- Before jury execution, require the exact released evaluatorq version whose `llm_jury(assignment="all")` result contains complete per-model and per-repetition detail, plus explicit approval for the paid calls. The intended run is three distinct judges with three repetitions per judge; a two-row smoke precedes the separately approved 450-call full run.
...
- A validated detailed jury artifact showing all three judges and all three repetitions, produced only after dependency and call-approval gates; none exists yet.
...
- The exact released evaluatorq package exposes complete per-model and per-repetition results for `llm_jury(assignment="all")`; do not substitute client-side pinned correctness repeats.
...
- Released-package compatibility evidence, a pre-run 450-call budget estimate, and explicit operator confirmation.
...
3. Review and integrate Task 4's resource schema, prompt, tests, retained historical deterministic definitions, and single subjective jury definition. Remove the tracked project ID in its separately scoped follow-up and review a fresh unified semantic plan only with authorization for that remote read; no hosted apply is authorized by this increment.
...
| Jury/repetition detail is assumed available from the pinned historical path | Disagreement and wobble are reconstructed from incomplete aggregates and become invalid | Stop at the package gate until a released evaluatorq version returns native three-model, three-repetition detail; validate it on two v4 rows, then seek explicit approval for the 450-call run |
...
1. Review and integrate Tasks 1-4 from `feature/decision-support-evaluation`, then rerun focused and full non-live tests, Ruff, resource compilation, and diff checks on local `main` before changing any status to `VERIFIED`.
