...
| Hosted resources and simulation | `ACTIVE` | An isolated task is implementing YAML resources, YAML-to-Orq SDK transformation/sync, Make targets, the row-aware local-tool bridge, and the 50-case corpus | Finish offline tests, reconcile live state safely, verify bridge, then freeze the corpus |
| Offline CI | `ACTIVE` | An independent task is adding network-free checks | Validate workflow isolation and merge without coupling it to live Orq work |
...
- Make hosted agent configuration reproducible from reviewed YAML and synchronize it to the existing Orq project without duplicates.
...
| Hosted desired state | `orq/resources/agents/`, `orq/resources/tools/`, later datasets/alignment resources | Reviewable agent, local function declarations, corpus, and accepted alignment artifacts |
| Resource transform/sync | `src/analytics_chatbot/orq_resources.py`, `orq_sync.py`, `scripts/sync_orq_resources.py`, `Makefile` | Validate YAML, build SDK requests, semantic diff, safe apply, repeat-run no-op verification |
...
| WS3a | `hosted-agent-yaml-sync` | `ACTIVE` | Agent/tool YAML, YAML-to-Orq SDK transform, idempotent Make target, remote reconciliation evidence | WS0; compatible SDK; project access | WS2, WS4 |
...
| WS4 | `offline-ci` | `ACTIVE` | Credential-free pytest and Ruff workflow | Integrated dependency set | WS2, WS3 |
...
- [ ] WS4: Finish offline CI independently and prove it makes no network calls and requires no secret.
...
- YAML defines the analytics agent and both local function declarations with no credential or hard-coded remote identifier.
- The transformation layer validates YAML shape, block-scalar prompts, cross-resource references, model capability, and local-only fields before constructing current Orq SDK entities.
- Selection uses stable human-readable keys; runtime-resolved IDs are not committed.
- The default Make target produces a semantic plan only. The explicit apply target operates only on the verified existing project, creates/updates in dependency order, never deletes, and refuses duplicates or project mismatch.
...
- Offline tests for YAML parsing, transformation, pagination, duplicate detection, create/update/no-op planning, project mismatch, sanitized errors, and mocked SDK calls.
- A reviewed dry-run summary and a post-apply human-readable inventory by stable resource name only.
...
- The target is the existing Orq project named `pydata2026`. Before WS3a began it was observed to contain no agent, tool, or dataset resources; treat that observation as historical because remote state is mutable.
...
- Resource order is tools first, then agent references, then accepted dataset rows. A failed partial apply must be safe to resume.
...
3. Integrate WS3a resource schemas/transformation/sync next. Resolve overlapping dependency-lock and README changes deliberately. Remove any hard-coded runtime IDs before acceptance. Review the semantic plan before any separately authorized apply.
...
5. Integrate WS4 whenever its workflow is rebased onto the final dependency commands; it must remain logically independent and network-free. If merged earlier, rerun it after dependency-lock changes from WS2/WS3.
...
| 2026-09-03 | Manage hosted agent/tools via YAML transformed through the Orq SDK with repeatable Make targets | Active; runtime IDs remain untracked |
...
| 2026-09-03 | Build offline CI as an independent workstream | Active; live checks remain protected/manual |
...
| 2026-09-03 | WS3 | Hosted YAML/SDK sync, Make targets, row-aware bridge, and 50-case corpus are active in an isolated task | Finish offline validation, remove tracked runtime IDs, then review remote plan before any apply |
| 2026-09-03 | WS4 | Offline CI is active independently | Keep credential-free and revalidate after dependency integration |
| 2026-09-03 | Planning | Consolidated status, acceptance evidence, security, remote-state, and integration order in this document | Update this table whenever status or decisions change |
...
2. WS3a owner: finish YAML/SDK sync tests, remove hard-coded remote IDs from tracked desired state, and produce a reviewed dry-run against `pydata2026`; perform apply only under the active task's explicit authorization.
...
4. WS4 owner: finish the offline workflow and reproduce its commands locally without credentials or network calls.
