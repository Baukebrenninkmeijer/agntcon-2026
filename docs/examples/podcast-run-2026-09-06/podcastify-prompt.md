...
The n8n step that turns the digest into the narration draft. Written by hand, never checked by
anything. Almost every rule in it is a consequence of the output being spoken rather than read —
though what it produces is not what the listener hears, because the podcast service rewrites it.
